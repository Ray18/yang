...
<table id="goodList" width="100%" lay-filter="user"></table>
    ...
    layui.use('table', function () {
        ...
        table.render({
            ...
            , cols: [[
                ...
                , {field: 'goodsNum', title: '数量', width: '17%', sort: true}
                , {field: 'goodsUnit', title: '单位', width: '20%'}
                , {
                    field: 'goodsType', title: '类型', width: '13%', templet: function (d) {
                        if (d.goodsType == 1) {
                            return '口罩'
                        }
                        if (d.goodsType == 2) {
                ...
                , {field: 'goodsUseDay', title: '预计使用天数', width: '13%'}
                , {field: 'goodsAccumulateNum', title: '累积使用数量', width: '13%'}
        ...
        var $ = layui.$, active = {
            ...
            add: function () {
                add('添加用户', '/qj/goods/showAddGood', 700, 450);
            },
            update: function () {
                var checkStatus = table.checkStatus('goodList')
        ...
        table.on('tool(user)', function (obj) {
            var data = obj.data;
            if (obj.event === 'detail') {
                detail('编辑用户', 'qj/goods/updateGood?id=' + data.id, 700, 450);
            } else if (obj.event === 'del') {
                layer.confirm('确定删除商品[<label style="color: #00AA91;">' + data.goodsName + '</label>]?', {
                    btn: ['删除']
                }, function () {
                    gooddel(data.id);
                });
            } else if (obj.event === 'edit') {
                update('编辑用户', 'qj/goods/updateGood?id=' + data.id, 700, 450);
            }
        });
